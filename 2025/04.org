...
Zoals zo vaak bij een [[id:d74b47b0-cd57-43c0-ae15-61e09c0d1955][Grid puzzle]] zet ik de input om naar coördinaten waar de relevante data staat. In dit geval een set met papier locaties.
...
Van alle buren kijken we hoeveel er daadwerkelijk in de originele grid zitten
(of er nog een papierrol is). In eerste instantie schreef ik dat, tijdens het
oplossen, als onderstaande:

#+begin_src python
if sum(1 for c in neighbors((x,y)) if c in grid) < 4:
#+end_src

Maar later heb ik de neigbors functie een set laten teruggeven en kon het door
de intersectie te bepalen:
...
Ook weer de walrus gebruikt zodat ik geen oneindige lus hoefde te definiëren met een break. zie het verschil tussen onderstaande loops.
