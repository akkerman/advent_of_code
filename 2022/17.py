...
def part_two(jetpattern):
    """ part two """
    # print, for 10k rock, the difference in height on each rock
    # in editor find repeating pattern
    ...
    initialRocks = len(init)
    initialHeight = sum(init)

    repeatRocks = len(repeat)
    repeatHeight = sum(repeat)

    repetitions, remainder = divmod(TARGET - initialRocks, repeatRocks)

    tailHeight = sum(repeat[0:remainder])

    result = initialHeight + repetitions * repeatHeight + tailHeight
