...
parse dirs path (["$", "cd", name]:xs) = parse (M.insertWith (+) (path ++ [name]) 0 dirs) (path ++ [name]) xs
parse dirs path (["$", "ls"]:xs)       = parse dirs path xs
parse dirs path (["dir", name]:xs)     = parse (M.insertWith (+) (path ++ [name]) 0 dirs) path xs
parse dirs path ([size, name]:xs)      = parse (addSizeToParents dirs path (atoi size)) path xs
    where
      ...
      addSizeToParents dirs xs size = addSizeToParents (M.insertWith (+) xs size dirs) (init xs) size
